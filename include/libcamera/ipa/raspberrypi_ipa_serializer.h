...
public:
		  ...
		  [[maybe_unused]] ControlSerializer *cs = nullptr)
	{
		...
		std::vector<uint8_t> hblankDelay;
		std::tie(hblankDelay, std::ignore) =
			IPADataSerializer<uint32_t>::serialize(data.hblankDelay);
		retData.insert(retData.end(), hblankDelay.begin(), hblankDelay.end());
		    ...
		    [[maybe_unused]] ControlSerializer *cs = nullptr)
	{
		...
		if (dataSize < 4) {
			LOG(IPADataSerializer, Error)
				<< "Failed to deserialize " << "hblankDelay"
				<< ": not enough data, expected "
				<< (4) << ", got " << (dataSize);
			return ret;
		}
		ret.hblankDelay = IPADataSerializer<uint32_t>::deserialize(m, m + 4);
		m += 4;
		dataSize -= 4;
