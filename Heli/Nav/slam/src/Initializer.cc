...
namespace ORB_SLAM2
{
    ...
    bool
    Initializer::Initialize(const Frame &CurrentFrame, const std::vector<int> &vMatches12, cv::Mat &R21, cv::Mat &t21,
    ...
    float
    Initializer::CheckHomography(const cv::Mat &H21, const cv::Mat &H12, std::vector<bool> &vbMatchesInliers,
                                 float sigma)
    {
        const int N = mvMatches12.size();
    ...
    void Initializer::Normalize(const std::vector<cv::KeyPoint> &vKeys, std::vector<cv::Point2f> &vNormalizedPoints,
                                cv::Mat &T)
    {
        float meanX = 0;
        float meanY = 0;
    ...
    int Initializer::CheckRT(const cv::Mat &R, const cv::Mat &t, const std::vector<cv::KeyPoint> &vKeys1,
