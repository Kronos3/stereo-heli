...
namespace ORB_SLAM2
{
    ...
    void Optimizer::LocalBundleAdjustment(KeyFrame* pKF, bool* pbStopFlag, Map* pMap)
    {
        ...
        {
            std::vector<MapPoint*> vpMPs = lLocalKeyFrame->GetMapPointMatches();
            for (auto pMP: vpMPs)
            {
                if (pMP)
                    if (!pMP->isBad())
        ...
        {
            std::map<KeyFrame*, size_t> observations = lLocalMapPoint->GetObservations();
            for (auto &observation: observations)
            {
                                           ...
                                           const bool &bFixScale)
    {
        ...
        for (const auto &LoopConnection: LoopConnections)
        {
            ...
            const std::set<KeyFrame*> &spConnections = LoopConnection.second;
